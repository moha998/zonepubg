...
const AuthModal = React.memo(({
  ...
  const handleLogin = async () => {
    ...
    const email = loginName.trim().includes('@') 
      ...
      : `${loginName.trim().toLowerCase()}@pubgzone.local`;
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        ...
        showNotification(translateFirebaseError(error) || 'فشل إرسال الرابط. يرجى المحاولة لاحقاً', 'error');
      }
    } finally {
      setIsLoading(false);
    }
              ...
              {activeTab === 'login' ? (
                  ...
                  <div className="flex items-center justify-between">
                    <button
                      ...
                      className={`flex items-center gap-2 text-sm transition-all focus:outline-none ${
                        rememberLogin ? 'text-primary' : 'text-slate-400 hover:text-white'
                      }`}
                    >
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${
                        rememberLogin ? 'bg-primary/20 border-primary text-primary' : 'bg-black/20 border-white/10 text-transparent'
                      }`}>
                        <Check size={14} />
                      </div>
                ...
                >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 text-sm transition-all focus:outline-none ${
                        rememberRegister ? 'text-primary' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${
                        rememberRegister ? 'bg-primary/20 border-primary text-primary' : 'bg-black/20 border-white/10 text-transparent'
                    }`}>
                      <Check size={14} />
                    </div>
                    <span className="font-medium">
