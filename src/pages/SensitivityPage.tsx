...
import { Sparkles, Smartphone, Loader2, Zap, Crosshair, Target, ChevronDown, Shield, Settings2, Pencil, CheckCircle2, Plus, Copy, Eye, RefreshCw, Star, Send, Activity } from 'lucide-react';
...
export default function SensitivityPage({
  ...
  const { currentDevices, currentPlayers, dbDevices, dbPlayers, sensitivityRatings, updateDevice, updatePlayer, addDevice, addPlayer } = useSensitivity();
  ...
  const [sensitivityCategory, setSensitivityCategory] = useState<"devices" | "players">("devices");
  ...
  // Initialize selected device when data loads
  useEffect(() => {
    if (!selectedDevice) {
      if (sensitivityCategory === "devices" && currentDevices.length > 0) {
        setSelectedDevice(currentDevices[0]);
      } else if (sensitivityCategory === "players" && currentPlayers.length > 0) {
        setSelectedDevice(currentPlayers[0]);
      }
    }
  }, [currentDevices, currentPlayers, sensitivityCategory, selectedDevice]);
